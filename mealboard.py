...
def get_config():
    ...
    parser.add_argument("--email", type=str, help="Mealboard email address.")
    ...
    # Note: Using MEALBOARD_EMAIL instead of USERNAME to avoid Windows system variable conflict
    config = {
        "email": os.getenv("MEALBOARD_EMAIL", args.email or ""),
        "password": os.getenv("MEALBOARD_PASSWORD", args.password or ""),
        "base_url": os.getenv("MEALBOARD_BASE_URL", args.base_url or "https://server.mealboardapp.com/"),
    }

    if not config["email"]:
        sys.exit("Error: Email must be set via .env file or command line (--email).")
...
def save_recipes(recipes, bearer, req, user):
    output_dir = "./recipes"
    ...
    with tqdm(total=len(recipes), desc="Processing Recipes", unit="recipe") as pbar:
        for recipe in recipes:
            try:
                ...
                url = f"{base_url}mealboard/getrecipe/{user}"
...
if __name__ == "__main__":
    ...
    user = config["email"]
    r = req.post(base_url + 'mealboard/login', json=payload)

    check_response(r, "Unable to log in:")

    # Extract bearer token from login response
    login_data = r.json()
    bearer = login_data.get('token', '')
    if not bearer:
        sys.exit("Error: Failed to retrieve bearer token from login response.")
    ...
    save_recipes(recipe_list, bearer, req, user)
